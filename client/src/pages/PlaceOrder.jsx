...
function PlaceOrder() {
  ...
  const fetchUserProfile = async (token) => {
    try {
      const response = await api.get('/auth/me', { // Removed /api prefix
  ...
  const fetchGallery = async () => {
    try {
      const res = await api.get('/gallery'); // Removed /api prefix
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/orders', payload, { // Removed /api prefix
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
