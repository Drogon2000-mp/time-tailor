...
function AdminDashboard() {
  ...
  const [cancelReason, setCancelReason] = useState(''); // This state is used
  ...
  const fetchOrders = async () => {
    try {
      const res = await api.get('/orders/admin/orders'); // Removed /api prefix
  ...
  const fetchAppointments = async () => {
    try {
      const res = await api.get('/appointments/admin/all'); // Removed /api prefix
  ...
  const fetchGallery = async () => {
    try {
      const res = await api.get('/gallery/admin/all'); // Removed /api prefix
  ...
  const fetchFabrics = async () => {
    try {
      const res = await api.get('/fabrics/admin/all'); // Removed /api prefix
  ...
  const fetchProducts = async () => {
    try {
      const res = await api.get('/products/admin/all'); // Removed /api prefix
  ...
  const uploadGalleryImage = async (e) => {
    ...
    try {
      ...
      const createRes = await api.post('/gallery', {
  ...
  const uploadFabricImage = async (e) => {
    ...
    try {
      ...
      const uploadRes = await api.post('/fabrics/upload', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      ...
      const createRes = await api.post('/fabrics', {
                      ...
                      <tr key={order._id}>
                        ...
                        <td>
                          {order.items?.map((item, idx) => (
                            <div key={idx} className="admin-order-item-mini" style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                              <img 
                                src={(item.image && item.image.trim()) ? item.image : '/images/product-placeholder.jpg'} 
                                alt="" 
                                style={{ width: '35px', height: '35px', borderRadius: '4px', objectFit: 'cover' }} 
                                onError={(e) => { e.target.src = '/images/product-placeholder.jpg'; }}
                              />
                              <small>{item.serviceName} (x{item.quantity || 1})</small>
                            </div>
                          ))}
                        </td>
                        <td>
                        ...
                        <td>
                          {order.deliveryAddress ? (
                            <div className="delivery-info">
                              ...
                              {order.deliveryAddress.landmark && `${order.deliveryAddress.landmark}, `}📞 {order.deliveryAddress.deliveryPhone}
                              {order.esewaPaymentId && <div><small>💳 eSewa: {order.esewaPaymentId}</small></div>}
                            </div>
                          ) : 'N/A'}
                        </td>
                ...
                products.map(product => (
                  <div key={product._id} className="product-card-admin">
                    ...
                    <div className="product-info">
                      ...
                      <p className="product-price"> ₹ {product.basePrice?.toLocaleString('en-IN') || '0'}</p>
